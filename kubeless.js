...
module.exports = function kubeless(options){
  ...
  app.all('*', (req, res) => {
    ...
    if (req.method === 'OPTIONS') {
      ...
      const modPath = path.join(modRootPath, `${modName}.js`).replace(/\\/g,"/");
      const libPath = path.join(modRootPath, 'node_modules').replace(/\\/g,"/");
      const pkgPath = path.join(modRootPath, 'package.json').replace(/\\/g,"/");
